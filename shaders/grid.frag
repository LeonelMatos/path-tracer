...
vec4 grid(vec3 grid_pos, float scale) {
    vec2 coord = grid_pos.xy * scale;
    ...
    //axis subtle color
    if(abs(grid_pos.y) < deriv.x * 2.5)
        color = vec4(0.6, 0.25, 0.25, alpha);
    if(abs(grid_pos.x) < deriv.y * 2.5)
        color = vec4(0.25, 0.6, 0.25, alpha);
...
void main() {
    ...
    vec3 world_pos = nearPoint + t * (farPoint - nearPoint);

    vec4 g = grid(world_pos, 1.0);

    //Distance fade
    float dist = length(world_pos.xy - camera_pos.xy);
